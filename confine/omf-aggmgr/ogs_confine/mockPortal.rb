module Confine
	class MockPortal
		...
		def createSlice
			result = Hash.new
			@sliceId += 1
			result[:testbed] = "#{DOMAIN_PREFIX}#{@sliceId}"
			result
		end
		...
		def mockSliver(sliceid, hostname)
			...
			sliver[:location][:name] = "VirtualWorld.#{sliver[:location][:x]}.#{sliver[:location][:y]}.#{sliver[:location][:z]}"
			sliver[:location][:testbedname] = "#{sliceid}"
